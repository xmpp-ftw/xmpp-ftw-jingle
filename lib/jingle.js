...
Jingle.prototype.response = function(data) {
    if (!data.to)
...
Jingle.prototype._sendErrorResponse = function(data) {
    var stanza = new builder.Element('iq', {
        to: data.to, id: data.id, type: 'error'
    })
...
Jingle.prototype.request = function(data, callback) {
    ...
    var stanza = this._getStanza(data, 'set', data.jingle.action)
